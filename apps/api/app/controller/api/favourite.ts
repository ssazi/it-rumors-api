...
export default class Favourite extends Controller {
  async get() {
    ...
    const data = await app.model.Favourite.get(params)
    if (data)
  ...
  async list() {
    const { ctx, app } = this
    const result = await app.model.Favourite.query(ctx.request.query)
  ...
  async add() {
    ...
    const d = await app.model.Favourite.get({ aid, sid, uid })
    if (r.uid === uid)
    ...
    const result = await app.model.Favourite.add(params)
    if (result) {
      r?.increment('favourite_count', { silent: true })
      await app.model.Feed.add({ ip, sid, uid, type: feedTypeBig.FAVOURITE, aid })
    }
  ...
  async edit() {
    const { ctx, app } = this
    const result = await app.model.Favourite.edit(ctx.request.body)
    ctx.helper.success(ctx, { data: result })
  }
  ...
  async delete() {
    ...
    const result = await app.model.Favourite.delete({ id })
    if (result) {
      await app.model.Feed.delete({ aid: id, uid, sid: modelName.FAVOURITE, type: feedTypeBig.FAVOURITE })
