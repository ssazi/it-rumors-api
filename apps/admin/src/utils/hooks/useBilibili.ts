...
export default () => {
  ...
  const getBilibili = async (id: string, findMcid: any) => {
    setBiliLoading(true)
    if (!id) {
    ...
    params.first_date = publish.pub_date ? publish.pub_date : null
    const day = dayjs(params.first_date).day()
    params.weekday = params.first_date ? [String(day === 0 ? 7 : day)] : null
